...
int intSize() {
  ...
  intPtr1 = intArray;       //first Int (address of the integer)
  intPtr2 = intArray + 1;   //second Int 
  return ((long int) intPtr2 - (long int) intPtr1); //subtract difference of the two ints treating them as long ints to compute size of int
}
...
void swapInts(int * ptr1, int * ptr2) {
  // Your code here
  int temp = *ptr1;     //everything is the same except the dereference (same as swap from Java)
 *ptr1 = *ptr2;        //we want values so must dereference pointers
 *ptr2 = temp;
}
...
int withinSameBlock(int * ptr1, int * ptr2) {
  // Your code here
  //Two pointers are within the same block of memory of address is same except last 6 bits
  //mask out last 6 bits
  //return 1 if stored and 0 if not
  int a = ((long int)ptr1 - 1) >> 6;
  int b = ((long int)ptr2 - 1) >> 6;
  return !(a^b);
}
      ...
      for (i = 0; i < arrLength-1; i++)
      {
          ...
          for (j = i+1; j < arrLength; j++)
            if (arr[j] < arr[min_index])
              min_index = j;
            //got error using swap() so just hard coded again
           temp = arr[i];
           arr[i]=arr[min_index];
           arr[min_index]= temp;
      }
