...
int intSize() {
  ...
  intPtr1 = intArray;       //gets the address of the first int
  intPtr2 = intArray + 1;   //gets address of the second int

  address1 = intPtr1;  //So you need to create new variables that actually hold the address value
  address2 = intPtr2;  //If you just try to subtract the pointers, it will return 1 as it subtracting not as the addresses.

  return address2 - address1; //the addresses subtracted should give us the size of the int
}
...
int doubleSize() {
  ...
  doubPtr1 = doubArray;       //gets the address of the first double
  doubPtr2 = doubArray + 1;   //gets address of the second double

  address1 = doubPtr1;  //Same as before
  address2 = doubPtr2;

  return address2 - address1; //the addresses subtracted should give us the size of the double
}
...
int pointerSize() {
  ...
  ptrPtr1 = ptrArray;       //gets the address of the first pointer
  ptrPtr2 = ptrArray + 1;   //gets the address of the second double
  ...
  address1 = ptrPtr1;  //Same as before
  address2 = ptrPtr2;

  return address2 - address1;     //the addresses subtracted should give us the size of the pointer
...
void swapInts(int *ptr1, int *ptr2) {
  ...
  int temp = *ptr1;     //Basically the general swap method
  *ptr1 = *ptr2;        //except we want the values so we dereference
  *ptr2 = temp;         //the pointers as we go
}
...
int changeValue() {
  ...
  int * intPtr2 = intPtr1 + 5; //takes us to intArray[5]
...
int withinSameBlock(int *ptr1, int *ptr2) {
  // Your code here
  int address1 = ptr1;     //address of 1st pointer
  int address2 = ptr2;     //address of 2nd pointer

  //The lower 6 bits of the pointer are the address of a byte within the 64-byte block
  //The upper 26 bits are the block address --> We want the block address to be the same
  //so we just want to check this basically

  //we want to compare the upper 26 bits because that will show they point to memory block
  //thats why we shift away the less significant 6 bits because we don't want to compare those
  return (((address1 >> 6) << 6) == ((address2 >> 6) << 6));
}
...
int withinArray(int * intArray, int size, int * ptr) {
  // Your code here
    int t = 1;
    int f = 0;
    while(size--) {                 //basically go through the array by decreasing the size until 0
        if(ptr == intArray) return t; // check if the pointer is
        intArray++;             //increment through the array
    }
    return f;     //return 0 if never found



    while(size) {
        if(ptr == intArray) return 1;
        intArray++;
        size--;
    }
    return 0;
...
int stringLength(char * s) {
  // Your code here
    ...
    int count = 0;
    while(*s != '\0'){      //loop thru s until u reach the null character
        count++;        //increment count as u go
        s++;
    }
    return count;
...
int endianExperiment(int* ptr) {
  char *byteptr = ptr;
  // Your code here
  //Basically we want to set the pointer to each byte of 449449
  //and then increment after doing
  *byteptr = 0xA9;
  byteptr++;
  *byteptr = 0xDB;
  byteptr++;
...
void selectionSort(int arr[], int arrLength) {
  int i, j, min_index;
  // Your code here
  //Following the pseudocode from above
  for(i = 0; i < arrLength-1; i++){
      ...
      for(j = i+1; j < arrLength; j++){
          ...
          if(*(arr + min_index) > *(arr + j)){
              min_index = j;  //basically looking for smallest index to do the swap
              // printf("%i \n\n",min_index);
          }
      }
